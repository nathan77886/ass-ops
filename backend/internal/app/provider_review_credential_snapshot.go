...
func RecordProviderReviewAttemptCredentialSnapshot(ctx context.Context, store *Store, opts ProviderReviewAttemptCredentialSnapshotOptions) (map[string]any, error) {
	...
	written, err := recordAssetStatusSnapshotIfChanged(ctx, store.Gorm, assetID, status, health, "provider review attempt credential snapshot recorded", snapshot)
	if err != nil {
		return nil, fmt.Errorf("recording provider review attempt credential snapshot recorded: %w", err)
	}
