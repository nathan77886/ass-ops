...
func RecordProviderReviewAttemptResponseSnapshot(ctx context.Context, store *Store, opts ProviderReviewAttemptResponseSnapshotOptions) (map[string]any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("starting provider review attempt response snapshot transaction: %w", err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	if _, err := tx.ExecContext(ctx, `SELECT pg_advisory_xact_lock(hashtext($1::text), hashtext($2::text))`, assetID, status); err != nil {
		return nil, fmt.Errorf("locking provider review attempt response snapshot asset: %w", err)
	}
	execResult, err := tx.ExecContext(ctx, `
		INSERT INTO asset_status_snapshots(asset_id, status, health, summary, raw)
		SELECT $1, $2, $3, 'provider review attempt response snapshot recorded', $4
		WHERE NOT EXISTS (
			SELECT 1
	...
	if err != nil {
		return nil, fmt.Errorf("inserting provider review attempt response snapshot: %w", err)
	}
	written := 0
	rowsAffectedWarning := ""
	...
	if err := tx.Commit(); err != nil {
		return nil, fmt.Errorf("committing provider review attempt response snapshot: %w", err)
	}
	committed = true
	...
	if rowsAffectedWarning != "" {
		...
		result["provider_review_attempt_response_snapshot_written"] = false
...
func providerReviewAttemptResponseSnapshotPayload(attempt, ledger map[string]any, assetObserved bool) map[string]any {
	...
	return map[string]any{
		...
		"contains_branch_name":                        false,
		"contains_file_content":                       false,
		"status_snapshot_written":                     assetObserved && candidateMatches && len(responsePlan) > 0 && len(resultPlan) > 0 && len(transactionPlan) > 0 && len(providerCallBoundaryPlan) > 0,
