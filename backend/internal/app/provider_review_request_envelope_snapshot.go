...
func RecordProviderReviewAttemptRequestEnvelopeSnapshot(ctx context.Context, store *Store, opts ProviderReviewAttemptRequestEnvelopeSnapshotOptions) (map[string]any, error) {
	...
	written, err := recordAssetStatusSnapshotIfChanged(ctx, store.Gorm, assetID, status, health, "provider review attempt request envelope snapshot recorded", snapshot)
	if err != nil {
		return nil, fmt.Errorf("recording provider review attempt request envelope snapshot recorded: %w", err)
	}
