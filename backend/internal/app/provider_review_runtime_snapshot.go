...
func RecordProviderReviewAttemptRuntimeSnapshot(ctx context.Context, store *Store, opts ProviderReviewAttemptRuntimeSnapshotOptions) (map[string]any, error) {
	...
	written, err := recordAssetStatusSnapshotIfChanged(ctx, store.Gorm, assetID, status, health, "provider review attempt runtime snapshot recorded", snapshot)
	if err != nil {
		return nil, fmt.Errorf("recording provider review attempt runtime snapshot recorded: %w", err)
	}
