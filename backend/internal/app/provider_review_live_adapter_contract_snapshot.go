...
func RecordProviderReviewAttemptLiveAdapterContractSnapshot(ctx context.Context, store *Store, opts ProviderReviewAttemptLiveAdapterContractSnapshotOptions) (map[string]any, error) {
	...
	written, err := recordAssetStatusSnapshotIfChanged(ctx, store.Gorm, assetID, status, health, "provider review attempt live-adapter contract snapshot recorded", snapshot)
	if err != nil {
		return nil, fmt.Errorf("recording provider review attempt live-adapter contract snapshot recorded: %w", err)
	}
