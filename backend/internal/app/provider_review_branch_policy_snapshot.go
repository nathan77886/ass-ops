...
func RecordProviderReviewAttemptBranchPolicySnapshot(ctx context.Context, store *Store, opts ProviderReviewAttemptBranchPolicySnapshotOptions) (map[string]any, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("starting provider review attempt branch policy snapshot transaction: %w", err)
	}
	committed := false
	defer func() {
		if !committed {
	...
	if _, err := tx.ExecContext(ctx, `SELECT pg_advisory_xact_lock(hashtext($1::text), hashtext($2::text))`, assetID, status); err != nil {
		return nil, fmt.Errorf("locking provider review attempt branch policy snapshot asset: %w", err)
	}
	execResult, err := tx.ExecContext(ctx, `
		INSERT INTO asset_status_snapshots(asset_id, status, health, summary, raw)
		SELECT $1, $2, $3, 'provider review attempt branch policy snapshot recorded', $4
		WHERE NOT EXISTS (
			SELECT 1
	...
	if err != nil {
		return nil, fmt.Errorf("inserting provider review attempt branch policy snapshot: %w", err)
	}
	written := 0
	rowsAffectedWarning := ""
	...
	if err := tx.Commit(); err != nil {
		return nil, fmt.Errorf("committing provider review attempt branch policy snapshot: %w", err)
	}
	committed = true
	...
	if rowsAffectedWarning != "" {
		...
		result["provider_review_attempt_branch_policy_snapshot_written"] = false
