...
- Asset dependency paths, manual graph edges, saved graph views, and cross-project graph search exist, and direct project/repository/remote/RepoSyncAsset lifecycle/run queue plus worker-status writes and stale worker recovery, provider-account management, webhook-connection management, webhook-event delivery read-model writes, AI-runtime management, worker-node registration, Argo/SSH/manual-relation create/delete, template-worker project creation/completion/retry writes, Argo app sync read-model writes, and GitHub Actions pipeline read-model writes now sync the canonical ledger in transaction, but transactionally maintained canonical writes across every asset-producing path are still deferred.
