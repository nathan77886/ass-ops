...
`make docker-pg18-runtime-up` rebuilds the long-lived local Docker API runtime from current backend binaries, applies migrations to the host PG18 `assops` database, seeds demo data, recreates the `assops-live-pg18-*` API containers, and runs the Docker PG18 runtime check. It uses Docker only, does not create a local web container, and does not create or mutate k3s resources.
`make docker-pg18-runtime-check` verifies the long-lived local Docker API runtime backed by the host PG18 container: the `assops-live-pg18-*` API containers are running, the PG18 `assops` database has the latest checked-in schema, and the gateway API smoke passes through the gateway port directly. It reports the Cloudflare `/api` route status separately so a static frontend response is not mistaken for a local Docker failure.
`make cloudflare-docker-route-check` runs the same Docker/PG18 runtime checks but fails until `https://ass-ops-api.4nathan.com/api/auth/me` returns gateway JSON instead of static frontend HTML. Use it after updating the Cloudflare Tunnel routing to point the API hostname at the Docker gateway port. The frontend hostname `https://ass-ops.4nathan.com` remains bound to the `ass-ops` Worker.
