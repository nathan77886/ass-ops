...
- External Gitea/GitHub project-template repository provisioning is first-pass only; provider accounts cover basic account selection, manual token-env rotation with due/soon/fresh visibility, manual token/API health checks, protected-branch push avoidance, sanitized failure diagnostics, and operator-triggered provisioning retry, while automated token rotation and provider-specific protected-branch reconciliation remain limited.
