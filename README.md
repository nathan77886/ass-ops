...
- Provider review adapter tests now lock the GitHub/Gitea disabled adapter surface across runtime selection, request builder, provider client, execute method, response handler, and live-adapter contract metadata without materializing provider URLs, credentials, refs, branch names, request/response bodies, headers, or file content.
