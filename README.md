...
CI validates workflow syntax/semantics with `actionlint`, secret scanning with Gitleaks, Go tests, `go vet`, frontend build, Compose config, the gateway API smoke script self-test, database backup/restore rehearsal, Helm lint/template for default, test, and production example values, the test Helm readiness plan, a disposable kind-based Helm install smoke test with `/healthz` build-metadata verification, Docker image builds, and `govulncheck`.
...
- Project template GitHub/Gitea repository provisioning reads provider account token environment names, never stored token values. Allowed names are `ASSOPS_GITHUB_TEMPLATE_TOKEN`, `ASSOPS_GITEA_TEMPLATE_TOKEN`, or provider-scoped `ASSOPS_TEMPLATE_PROVIDER_TOKEN_GITHUB_*` / `ASSOPS_TEMPLATE_PROVIDER_TOKEN_GITEA_*`; local/private provider API hosts are blocked unless `ASSOPS_ALLOW_LOCAL_TEMPLATE_PROVIDER_API=true` is explicitly set for development. Provider Accounts include manual token-env rotation, safe automated token-env rotation execution from ready candidate metadata, a manual health check that verifies the configured token environment variable against the provider `/user` API, and sanitized status metadata. Template run results surface sanitized provider provisioning diagnostics such as provider type, HTTP status, owner, repository name, provider message, structured repository reconciliation summaries, and approval-gated provider review execution requests that still keep provider API mutation disabled. Approval payloads include a content-redacted starter-file staging summary with file count, id, path, kind, and status only, plus a redacted provider API request plan with endpoint keys and payload-shape names and a provider review preflight reconciliation summary for credential/gate/adapter/operation status, including a redacted adapter contract for required capabilities and scopes and a redacted adapter execution blueprint for the future request builder/response handler/idempotency ledger boundary, but no URL, token, token-env name, repository name, or file content. Approved provider review attempts persist redacted request and response-diagnostic summaries with the planned payload builder, response handler, execution status, idempotency requirements, success status class, and retryable status classes for each operation; attempt response status is limited to `pending`, `success`, `retryable`, `failed`, or `blocked`, with unknown values redacted as `blocked`. The attempt orchestration summary also exposes a redacted execution candidate with next operation, endpoint key, data-integrity metadata gates, explicit execution-blocker gates for the unimplemented adapter and unarmed mutation, a redacted branch policy plan that blocks default/protected branch direct writes before review-branch PR/MR execution, a redacted request-validation preflight summary for the future provider request boundary, and a nested live adapter contract plan that defines the future adapter input fields, output fields, error classes, status transitions, persisted field allowlist, and suppressed fields without materializing URLs, headers, bodies, refs, file content, or credentials. Green data-integrity gates do not mean provider mutation is allowed. `ASSOPS_ENABLE_PROVIDER_REVIEW_EXECUTION=true` records that provider review execution was explicitly requested, and `ASSOPS_ARM_PROVIDER_REVIEW_MUTATION=true` can mark the rehearsed mutation plan armed for audit, but actual branch creation, starter-file commits, and PR/MR API mutation remain disabled until the provider adapters are implemented. Template remotes marked `protected` create the external repository but skip starter-file pushes by default unless metadata explicitly sets `allow_protected_branch_push`.
...
- A standalone GitHub review-branch executor now has isolated tests for the future provider mutation shape: read base ref, create an `assops/review/...` branch, commit staged files to that branch, and open a pull request while keeping token and file content out of returned metadata. It is not wired into the provider-review approval ledger yet; live UI/API execution still remains disabled until the ledger lock, retry/cleanup, and sanitized result recording boundaries are connected.
- Provider review live launch plans now include a redacted review-branch ledger adapter plan that maps the standalone atomic executor shape onto the existing stepwise attempt ledger pipeline (`create_branch_ref`, `commit_starter_files`, `open_review_request`) without materializing provider URLs, token env names, branch names, repository refs, request/response bodies, headers, or file contents. It deliberately keeps live execution blocked because the database no-call constraints, stepwise ledger execution, mutation arming, provider send, and result recording are not unlocked yet.
