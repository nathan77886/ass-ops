...
- External Gitea/GitHub project-template repository provisioning is first-pass only; provider accounts cover basic account selection, manual token-env rotation with due/soon/fresh visibility, manual token/API health checks, protected-branch push avoidance, protected-branch strategy diagnostics, sanitized failure diagnostics, structured repository reconciliation guidance, and operator-triggered provisioning retry, while automated token rotation and provider API-backed PR/MR reconciliation remain limited.
