...
- Asset dependency paths, manual graph edges, saved graph views, and cross-project graph search exist, and RepoSyncAsset lifecycle writes now sync the canonical ledger in transaction, but transactionally maintained canonical writes across every asset-producing path are still deferred.
