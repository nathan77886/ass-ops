...
- Extend transactionally maintained canonical assets/relations beyond direct project/repository/remote/RepoSyncAsset lifecycle/run queue writes, worker status/repo-tag writes, and stale worker recovery, provider account management, webhook connection/delivery-health writes, webhook event delivery read models, AI runtime management, worker node registration, Argo connection/status/read models, SSH create paths, manual relation maintenance, template-worker project creation/completion/retry writes, and GitHub Actions pipeline read models to the remaining asset-producing CRUD paths.
