...
- No Redis-backed queue/lock/pub-sub yet; worker queue is PostgreSQL only, though worker jobs now appear as canonical assets linked to their operation run and assigned node, and the UI exposes queue/node health summaries for operator triage.
